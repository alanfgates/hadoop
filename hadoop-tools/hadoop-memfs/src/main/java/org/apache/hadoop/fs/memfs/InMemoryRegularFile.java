...
class InMemoryRegularFile extends InMemoryFile {
  ...
  InMemoryRegularFile(InMemoryDirectory parent, String name, FsPermission perms, String owner,
  ...
  FileStatus stat() {
    return new FileStatus(getLen(), false, 1, getLen(), getLastModificationTime(),
        getLastAccessTime(), getPerms(), getOwner(), getGroup(), null,
        InMemoryFs.addSchemeToPath(getPath()));
  }

  @Override
  void dump(FileSystem otherFs, Path base) throws IOException {
    Path ourPath = InMemoryFs.mergePathsIntelligently(base, getPath());
    FSDataOutputStream out = otherFs.create(ourPath, false);
    out.write(data);
    out.close();
  }
